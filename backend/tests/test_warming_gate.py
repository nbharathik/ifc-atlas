...
class TestWarmingEnvelopeTiers:
    def test_viewer_tool_exempt_when_warming(self):
        ...
        # highlight_elements is read_viewer - runs without IfcOpenShell
        assert tool_tier("highlight_elements")[0] == "read_viewer"
        assert warming_envelope("highlight_elements") is None
    ...
    def test_read_model_tool_gated_when_warming(self):
        readiness_service.mark_ifcopenshell_warming()
        envelope = warming_envelope("get_model_stats")
        assert envelope is not None
        assert envelope["warming"] is True
        assert envelope["ifcopenshell"] == "warming"
        assert envelope["tool"] == "get_model_stats"
    ...
    def test_gate_clears_on_ready(self, monkeypatch):
        ...
        readiness_service.mark_ifcopenshell_warming()
        assert warming_envelope("rename_element") is not None
        ...
        assert warming_envelope("rename_element") is None
        assert warming_envelope("get_model_stats") is None
        assert warming_envelope("highlight_elements") is None
...
class TestExecuteToolGate:
    def test_execute_tool_returns_envelope_for_gated_tier(self):
        ...
        readiness_service.mark_ifcopenshell_warming()
        result = execute_tool("rename_element", {"express_id": 1, "new_name": "x"})
    ...
    def test_execute_tool_envelope_not_memoised(self):
        ...
        readiness_service.mark_ifcopenshell_warming()
        first = execute_tool("get_model_stats", {})
        second = execute_tool("get_model_stats", {})
    ...
    def test_execute_tool_skips_envelope_for_viewer_tier(self):
        ...
        """
        readiness_service.mark_ifcopenshell_warming()
        result = execute_tool("highlight_elements", {"element_ids": [1, 2, 3]})
