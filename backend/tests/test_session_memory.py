...
def test_update_returns_false_on_empty_result():
    m = mem()
    assert m.update("get_model_stats", None) is False
    assert m.update("get_model_stats", {}) is False


def test_update_accepts_json_string():
    m = mem()
    payload = json.dumps({"total_elements": 149, "storeys": ["Ground Floor"]})
    changed = m.update("get_model_stats", payload)
    assert changed is True


def test_update_extracts_element_count():
    m = mem()
    m.update("get_model_stats", {"total_elements": 149})
    block = m.get_block()
    # Not enough facts yet (only 1)
    assert block == ""
    # Add another fact
    m.update("get_storeys", {"storeys": ["Ground Floor", "First Floor"]})
    block = m.get_block()
    assert "149" in block
...
def test_update_extracts_storeys():
    m = mem()
    m.update("get_model_stats", {"total_elements": 50, "storeys": ["L1", "L2"]})
    block = m.get_block()
    assert "L1" in block
    assert "L2" in block
...
def test_update_deduplicates_by_key():
    m = mem()
    m.update("get_model_stats", {"total_elements": 100, "storeys": ["A"]})
    m.update("get_model_stats", {"total_elements": 200, "storeys": ["A"]})
    block = m.get_block()
    assert "200" in block
...
def test_update_search_elements():
    m = mem()
    m.update("get_model_stats", {"total_elements": 10, "storeys": ["G"]})
    m.update("search_elements", {"query": "walls near exit", "elements": [], "total_count": 5})
    block = m.get_block()
...
def test_update_search_elements_semantic():
    m = mem()
    m.update("get_model_stats", {"total_elements": 10, "storeys": ["G"]})
    m.update("search_elements_semantic", {"query": "load-bearing", "results": [1, 2, 3]})
    block = m.get_block()
...
def test_update_quantities_summary():
    m = mem()
    m.update("get_model_stats", {"total_elements": 10, "storeys": ["G"]})
    m.update("get_quantities_summary", {"total_area_m2": 120.5, "total_volume_m3": 60.0})
    block = m.get_block()
...
def test_update_elements_by_storey():
    m = mem()
    m.update("get_model_stats", {"total_elements": 10, "storeys": ["G"]})
    m.update("get_elements_by_storey", {"storey_name": "Ground Floor", "total_count": 80})
    block = m.get_block()
...
def test_update_elements_by_type():
    m = mem()
    m.update("get_model_stats", {"total_elements": 10, "storeys": ["G"]})
    m.update("get_elements_by_type", {"ifc_type": "IfcDoor", "total_count": 12})
    block = m.get_block()
...
def test_block_empty_when_less_than_two_facts():
    m = mem()
    m.update("get_model_stats", {"total_elements": 10})
    assert m.get_block() == ""
...
def test_get_facts_list_capped_at_max():
    ...
    m.update("get_model_stats", {"total_elements": 10, "storeys": ["G"]})
    for i in range(20):
        m.update("get_elements_by_type", {"ifc_type": f"IfcFake{i}", "total_count": i})
...
def test_update_returns_false_when_same_value():
    m = mem()
    first = m.update("get_storeys", {"storeys": ["Ground Floor"]})
    second = m.update("get_storeys", {"storeys": ["Ground Floor"]})
