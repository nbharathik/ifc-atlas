...
# reference material the LLM consults before writing IfcOpenShell code or
# picking IFC entities - kept apart from the user's uploaded documents.
# Populated by ``scripts/fetch_reference_docs.py`` (plan E2) and queried
# through the ``get_docs`` tool (plan E3). See
# ``dev/docs/AI_BIM_EDITOR_MASTER_PLAN.md`` Workstream E.
#
# The IfcOpenShell API docs are indexed from the *installed* package's
# docstrings (version-matched by construction, no network/git needed),
# aggregated one document per API domain to stay under
# ``DocumentIndexService.MAX_DOCS`` and to keep the per-add index rebuild
# count low.

_REFERENCE_INDEX_DIR = _CONFIG_DATA_DIR / "reference_index"
